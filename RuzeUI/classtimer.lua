...
local CAST_SEPARATOR_COLOR = { 0, 0, 1, 1 } 								-- Sets cast separator color
...
local CLASS_FILTERS = {
		MONK = { 
			...
			player = {
				...
				CreateSpellEntry( 125359, false, { .2, .9, .7} ), -- Tiger Power
		...
		ROGUE = { 
			target = { 
				...
				CreateSpellEntry( 1943, false, {.97, 0, 0} ), -- Rupture
				CreateSpellEntry( 703, false, {.45, .01, 0} ), -- Garrote
				CreateSpellEntry( 79140 ), -- vendetta
				CreateSpellEntry( 16511 ), -- Hemorrhage
				CreateSpellEntry( 84617, false, {.8, .75, .71} ), -- Revealing Stike
			},
			player = {
				CreateSpellEntry( 13750 ), -- adrenalin stuff
				CreateSpellEntry( 32645, false, {.59, .93, .31} ), -- Envenom
				CreateSpellEntry( 2983 ), -- Sprint
				CreateSpellEntry( 5277 ), -- Evasion
				CreateSpellEntry( 1776 ), -- Gouge
				CreateSpellEntry( 51713 ), -- Shadow Dance
				CreateSpellEntry( 1966, false, {.6, .9, 1} ), -- Feint
				CreateSpellEntry( 73651, false, {.13, .54, .38} ), -- Recuperate
				CreateSpellEntry( 5171, false, {1, 1, .2} ), -- Slice and Dice
...
do
	...
	do
		...
		CreateAuraBar = function( parent )
			...
			icon:Point( iconAnchor1, result, iconAnchor2, -3, 3 )
			...
			stacks:SetFont(C.media.font, 11, "OUTLINE");
			...
			bar:Point( "TOPLEFT", result, "TOPLEFT", C["classtimer"].barheight+1, 0 );
			bar:Point( "BOTTOMRIGHT", result, "BOTTOMRIGHT", 0, 0 );
			result.bar = bar;
	
			if C["classtimer"].icon ~= true then
			--	icon:SetAlpha(0);
			--	icon:Hide();
			--	icon:Kill();
				icon:Point( "TOPLEFT", UIParent, "TOPLEFT", -100, 100 ); -- Why won't it die D:
				bar:Point( "TOPLEFT", result, "TOPLEFT", 0, 0 );
			end
			...
			spark:SetAlpha(0);
			result.spark = spark;
			if ( SPARK ) ~= true then spark:SetAlpha(0) end
			
			local barbg = CreateFrame("Frame", nil, result, nil);
			barbg:Height( BAR_HEIGHT )
			barbg:Point("LEFT", bar, "LEFT", 0, 0)
			barbg:Point("RIGHT", spark, "CENTER")
			barbg:CreateShadow("Default")
			if C["classtimer"].icon == true then barbg:Point("LEFT", bar, "LEFT", -C["classtimer"].barheight-1, 0) end
			
			local border = CreateFrame( "Frame", nil, barbg, nil );
			border:SetFrameStrata("BACKGROUND")
			border:Point("TOPLEFT", barbg, "TOPLEFT", -1, 1)
			border:Point("BOTTOMRIGHT", barbg, "BOTTOMRIGHT", 1, -1)
			border:SetBackdrop {
				bgFile = C["media"].normTex , edgeFile = C["media"].blank, edgeSize = 1,
			...
			name:SetFont(C.media.font, 11, "OUTLINE");
			...
			result.name = name;
			if C["classtimer"].name ~= true then name:Hide() end
			
			local time = bar:CreateFontString( nil, "OVERLAY", nil );
			time:SetFont(C.media.font, 11, "OUTLINE");
			time:SetJustifyH( "RIGHT" );
			time:Point( "LEFT", name, "RIGHT", 0, 0 );
			time:Point( "RIGHT", bar, "RIGHT", -TEXT_MARGIN, 0 );
			result.time = time;
			if C["classtimer"].time ~= true then time:Hide() end
	...
	local SetAuraBar = function( self, index, auraInfo )
		...
		if ( line == nil ) then
			...
			if ( index == 1 ) then
				line:Point( "TOPLEFT", self, "BOTTOMLEFT", 0, BAR_HEIGHT);
	...
	CreateAuraBarFrame = function( dataSource, parent )
		local result = CreateFrame( "Frame", "Classtimer", parent, nil );
